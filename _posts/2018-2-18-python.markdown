...
`tokenizers-0.21.0-cp39-abi3-manylinux_2_17_x86_64.manylinux2014_x86_64.whl`

`cp39`：c表明wheel包里有c编译的binary。p39就是python 3.9

`abi3`：表示用了稳定的ABI，能跨版本用，比如给python 3.10用。

`huggingface_hub-0.34.4-py3-none-any.whl`

none表示这是一个纯py文件的包，任意平台都能用。
...
setup.py提供了entry_points机制用于第三方插件扩展。这在vllm等项目中得到了广泛应用。

https://zhuanlan.zhihu.com/p/503252479

Python setup.py entry_points详解
