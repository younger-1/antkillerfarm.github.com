...
冲击波是空气被压缩导致的。而空气的压力是由单位体积内的粒子能量决定的。因此，如果当量E可以产生R半径的空气压缩球的话，要产生2R的压缩球，势必需要将E提升8倍才行。

而火球（Fire Ball）半径就要麻烦一些了。

一方面，辐射的光子，它不占空间，能量可叠加，且不改变传播方向。从点光源的波动模型可以知道，波前能量如果没有衰减的话，R球面的能量和2R球面的能量相等，但2R球面的面积是前者的4倍，因此如果要维持能量密度的话，E提升4倍即可。

另一方面，辐射源需要维持一定的温度才能释放可见光，而温度也就是粒子平均动能，正如上面冲击波半径的推导，E和R的立方成正比。

所以，火球半径中E和R的关系，显然在2和3之间。实际上，它的理论值就是2.5，其推导过程如下：

物理量：辐射功率$$[L]=\text{ML}^2\text{T}^{-3}$$，能量$$[E]=\text{ML}^2\text{T}^{-2}$$，密度$$\rho_0=\text{ML}^{-3}$$，距离$$r=\text{L}$$

设$$L = C \cdot E^a \rho_0^b r^c$$

量纲等式：

$$\text{ML}^2\text{T}^{-3} = (\text{ML}^2\text{T}^{-2})^a (\text{ML}^{-3})^b \text{L}^c$$

分项匹配指数：

M：$$1 = a + b$$

T：$$-3 = -2a \Rightarrow a=3/2$$

L：$$2 = 2a - 3b + c$$

代入 $$a=3/2$$，得 $$b=1-a = -1/2$$

$$2 = 2\cdot\frac32 - 3\cdot(-\frac12) + c$$

$$2 = 3 + \frac32 + c \Rightarrow c = 2 - \frac92 = -\frac52 = -2.5$$

得到：

$$\boldsymbol{L \propto r^{-2.5}}$$
